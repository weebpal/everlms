...
namespace Drupal\we_notification\Plugin\Derivative;
...
class GroupedEntityLinksDeriver extends DeriverBase implements ContainerDeriverInterface {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    foreach ($this->entityTypeManager->getDefinitions() as $entity_type) {
      ...
      if ($entity_type_id === 'we_notification') {
        $links["{$entity_type_id}.add"] = [
          'title' => t('Add'),
          'route_name' => 'entity.we_notification.add_page',
          'parent' => 'entity.we_notification.collection',
        ...
        if ($entity_type->getBundleEntityType() !== NULL) {
          foreach ($this->entityTypeBundleInfo->getBundleInfo($entity_type_id) as $bundle_id => $bundle) {
            $links["{$entity_type_id}.{$bundle_id}.add"] = [
              ...
              'route_parameters' => [
                'we_notification_type' => $bundle_id,
              ],
              'parent' => "we_notification.grouped_entity_links:{$entity_type_id}.add",
            ] + $base_plugin_definition;
          }
        }
      }
